...
BEGIN
      ...
      loop
        ...
        if trim(v_line) is not null then
          ...
          if LENGTH(v_line) > 2 then
            overHunderedX := to_number(substr(v_line, 1, 1));
            zeroCounter := zeroCounter + overHunderedX;
          end if;
          ...
          if dialPossition = 0 AND clicked = 0 then
            zeroCounter := zeroCounter + 1;
          end if;
          
            clicked := 0;
            possitionBefore := dialPossition;
